...
function FormGas() {
  return (
    <div className="rounded-xl border border-border bg-card p-6 shadow-card">
      ...
      <p className="mb-5 text-sm text-muted-foreground">Inserisci il valore visualizzato sul contatore del gas.</p>
      ...
      <form className="space-y-5" onSubmit={(e) => e.preventDefault()}>
        <div>
          ...
          <select
            ...
            className="w-full rounded-lg border border-input bg-background px-4 py-2.5 text-sm text-card-foreground focus:outline-none focus:ring-2 focus:ring-primary"
          >
            <option>G0002254 - VIA ESEMPIO 10 NAPOLI (NA)</option>
          </select>
        </div>

        <div className="max-w-xs">
          <label htmlFor="gas-lettura" className="mb-1 block text-sm font-medium text-card-foreground">Lettura contatore *</label>
          <input
            type="number"
            id="gas-lettura"
            placeholder="0"
